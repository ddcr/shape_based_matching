...
std::pair<int, int> detectTemplateLinemod(const std::string& filename, const cv::Mat& img)
{
    ...
    timer_local.out("extraiu fiducial");
...
void jabil_read_all_templates_and_match(std::string testdir)
{
    ...
    for(auto &f: filelist)
    {
        ...
        std::pair<int, int> matchingResult = detectTemplateLinemod(f.filename(), img);
    }
}
#else
